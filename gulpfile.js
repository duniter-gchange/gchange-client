...
const paths = {
  sass: ['./scss/cesium.app.scss', './scss/ionic.app.scss'],
...
function pluginSass() {
  ...
  const noFaviconFilter = filter(["!favicon.ico"]);
...
function webPluginCopyFiles() {
  ...
  return merge(
    ...
    gulp.src('./www/locale/**/*.*')
      .pipe(gulp.dest(targetPath + '/locale')),

    // Copy converse sounds files
    gulp.src('./www/sounds/**/*.*')
      .pipe(gulp.dest(targetPath + '/sounds'))
  );
}
...
function webExtensionZip() {
  ...
  return gulp.src(srcPath + '/**/*.*')
    .pipe(zip('gchange-v'+version+'-extension.zip'))
    .pipe(gulp.dest(distPath));
}

function webBuildSuccess(done) {
  var version = JSON.parse(fs.readFileSync('./package.json', 'utf8')).version;
  log(colors.green("Web artifact created at: 'dist/web/build/gchange-v" + version + "-web.zip'"));
  if (done) done();
}

function webExtBuildSuccess(done) {
  var version = JSON.parse(fs.readFileSync('./package.json', 'utf8')).version;
  log(colors.green("Web extension artifact created at: 'dist/web/build/gchange-v" + version + "-extension.zip'"));
  if (done) done();
}
...
const build = gulp.series(app, plugin);
